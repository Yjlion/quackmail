...
list rather than a changelog. Everything here is **released** — v1.0.1 or
...
Seven improvements on top of the rebuilt interface, shipped as PR #53.
...
The web interface rebuild, shipped as PR #52 — the release that took the front
end from "has the features" to "presents them".
...
Ten web UI pull requests (#40–#50): the i18n scaffolding, rich text in
Notes/Calendar/Blog, ACL checkboxes, note swatches, mail list density, the
date-format preference, the address-book picker, the centred login, the admin
config descriptions and the live Sieve builder.

**None of it was written up when it landed** — it is in `git log` and nowhere
else. This heading is here so the record is honest about the gap rather than
silently skipping from v1.0.0 to v0.8.0, and so the next release notices.
