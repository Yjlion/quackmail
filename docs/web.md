...
- **Webmail** at `/mail/` — the user's personal rooms as folders, a paged
  ...
  *Write*, *Reply*, *Reply all* and *Forward* fetch `/mail/compose` and swap it
  ...
  `cid:` part rather than a base64 blob inside the body. The editor is
  **Squire**, the contenteditable editor Fastmail runs its own mail client on;
  its toolbar tracks the caret, and a paste from a web page keeps its bold and
  its lists instead of being flattened to bare text. Whether formatting starts
  on is a preference rather than something the composer decides. A signature set
  in *Preferences* is appended to a new message and never to a resumed draft.

  With scripting off, or if the editor fails to load, the composer is the plain
  `<textarea>` it has always been and the *Formatted text* option is not
  offered — the server reads the same fields either way.
...
every route returns a complete page. There are three vendored dependencies:

| File | Version | Licence | Size |
|---|---|---|---|
| `http/assets/pico.css` | Pico CSS v2.1.1 | MIT | 93 KB |
| `http/assets/htmx.min.js` | htmx 2.x | Zero-Clause BSD | 51 KB |
| `http/assets/squire.js` | Squire (`squire-rte`) 2.4.9 | MIT | 60 KB |

All three were fetched once by hand and committed, on the same terms as
`core/src/psl_data.cpp`: the build fetches nothing, so it works offline, and
their CVEs are ours to track. None of them can carry a `LICENCE` file beside it
— `tools/gen_assets.py` serves *every* file in `http/assets/` and refuses any
extension outside its MIME table — so their licences live in their own banners
and this table is the record.

Squire is taken from upstream's committed `dist/squire.js` (the minified,
non-module build, which sets `window.Squire`), so vendoring it needs no build
step. **Squire's default HTML sanitizer calls a global `DOMPurify`, which is
not vendored**; `qc-compose.js` supplies its own `sanitizeToDOMFragment`
implementing the same allow-list as `SanitizeForCompose` in
`core/src/html_sanitize.cpp`. Those two lists have to be changed together — a
mismatch is not a security hole, because the server sanitizes regardless, but it
is formatting the editor shows and the message silently drops.

Pico and htmx load on every page; **Squire loads only on the two pages that can
open a composer** (`/mail/compose` and the mailbox), so it does not change the
cold-load cost of anything else. This server sends no `Content-Encoding`, so
those are the real transfer sizes.
