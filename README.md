...
![And over SSH, signed in with a key](screenshots/text-ssh-login.png)
...
SMTP (MX, submission, LMTP), IMAP, POP3, ManageSieve, NNTP, XMPP, telnet, SSH
(the BBS shell, plus SFTP), FTP, HTTP — and Citadel's own protocol on 504. Over
HTTP it also serves CalDAV, CardDAV, WebDAV file shares and JMAP, so a phone
syncs the same rooms a BBS client reads. It obtains and
